...
@implementation FZAVAddWatermarkCommand
...
- (void)performWithAsset:(AVAsset*)asset completion:(void(^)(FZAVCommand* avCommand))block{
    ...
    if ([[self.mutableComposition tracksWithMediaType:AVMediaTypeVideo] count] != 0) {
        ...
        [self watermarkWithType:FZWatermarkTypeImage videosize:videoSize position:CGRectZero];
    }
...
- (void)watermarkWithType:(FZWatermarkType)watermarkType videosize:(CGSize)videoSize position:(CGRect)position{
    
    CALayer* watermarkLayer = [CALayer layer];
    watermarkLayer.bounds = CGRectMake(0, 0, 256*1.0, 256*1.0);
    
    if (watermarkType == FZWatermarkTypeText) {
        CATextLayer *titleLayer = [CATextLayer layer];
        titleLayer.string = @"FZAV";
    ...
    }else if (watermarkType == FZWatermarkTypeImage){
